...
app.get("/investigations", (req, res) => {
  ...
  const { source, severity, determination, page, column, order, date } = req.query;
  ...
  let results = investigations.filter((inv) => {
    return (
      ...
      (!severity || inv.severity.toLowerCase() === severity.toLowerCase()) &&
      (!id || inv.id === id) &&
  ...
  if (order && column) {
    if (order === "ascending") {
      results = results.sort((a, b) => {
        const aValue = a[column].toLowerCase().replace(/\s/g, '');
        const bValue = b[column].toLowerCase().replace(/\s/g, '');
  ...
  if (date) {
    ...
    results = results.filter(result => {
      const newDate = moment(result.alertFiredTimestamp).format("YYYY-MM-DD");
  ...
  res.json({
    ...
    resultTotalCount: results.length,
    totalPages,
  });
});
